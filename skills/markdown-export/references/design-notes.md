...
**Word で行間が間延びする（1行が約1.8行分に開く）** — 2026-08-21・09-06・09-23 の3回、同じ症状が出た。原因は3テンプレートすべての `word/document.xml` の `sectPr` にあった `<w:docGrid w:type="lines" w:linePitch="360"/>`（文書グリッド。各行を行送り18ptの格子へ吸着させる）。Word はこの格子を優先するため、テンプレートの行間指定（meiryo の0.85行など）が効かず、10pt前後の本文でも1行18ptに固定される。前2回は個別の文書を後処理で直しただけで、テンプレートに手が入らず再発した。
