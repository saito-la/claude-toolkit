...
再試行・再分割で回復する（モデル格上げは 2026-08-08 のモデル単一化で廃止した）。
...
#
# 「出所を書かない」の制約は齋藤指示（2026-08-17）。summary は素材フォルダから切り出されて
# プロジェクトの正本ディレクトリに単独で置かれ、そこでは録音・文字起こしの存在を知らない
# 読み手が読む。所要時間・確度記号・「〜と聞こえる」といった痕跡は、その文書の性格を
# 「議事録」から「自動処理の中間生成物」へ引き下げる。確度の情報そのものは捨てず、
# 平文の「（未確認）」へ移す——PERSON-PROTOCOL R2（推測禁止）は要約でも効き続けるため、
# 未確認の固有名を確定表記に格上げしてはならない。痕跡は transcript・verbatim が保持する。
...
この要約は独立した議事録として読まれます。**録音の文字起こしから作られたことが分かる記載を、一切残さないでください。**

- **会議の所要時間を書かない**（「所要 約72分」等）
- タイムスタンプ・区間見出し（`## Part N`）・話者ラベル行（「話者A:」形式）を残さない
- **確度記号（◎ ○ △ ？）を使わない**
- 音声からの復元をうかがわせる表現を使わない（「〜と聞こえる」「〜と発言があった」「聞き取れなかった」「文字起こし」「録音」「音声」「トランスクリプト」等）
- **未確認の事項は削除せず、平文で「（未確認）」と注記する。推測で確定しない**（人名の漢字は特に）。確度の情報を落とすことと、出所を隠すことは別である
- 話者の比定が不確かな発言は、氏名を書かず内容だけを記すか「出席者から」と書く。確度の低い氏名を断定的に帰属させない

確度記号と出所の情報は transcript・verbatim 側が保持する。要約はそれらを引き継がない。

## 文字修飾を使わない

本文に太字（`**`）・斜体・下線・ハイライトを入れないでください。強調は語順・見出し・箇条書きの構造で示します。理由は、修飾が読み手の判断を先取りする書き手の主観であり、多用すると強調が均質化して意味を失うこと、および Word・Google Docs へ変換したときの体裁崩れの一因になることです。コード・パス・識別子・列名の `` ` `` は修飾ではなく種別の表示なので使ってかまいません。
...
- 話者名は段落冒頭または文中で自然に示す（例「古賀部長より：…」）。確度記号は付けない（「出所を書かない」の制約に従う）
- 意味のない相槌のみの行は削除する
- 同じ内容を繰り返さない
...
- 時系列と論理の流れを保つ
- **出所を書かない。** 確度記号（◎ ○ △ ？）・所要時間・タイムスタンプ・区間見出し・話者ラベル行・音声由来をうかがわせる表現（「〜と聞こえる」「〜と発言があった」「文字起こし」等）は、区間要約に残っていればここで落とす。未確認の事項は削除せず平文の「（未確認）」に置き換える
- **文字修飾を使わない。** 太字（`**`）・斜体・下線・ハイライトは、区間要約に残っていればここで落とす
- 出力はMarkdownのみ（説明文不要）
...
- **加筆に出所への言及を混ぜない**（「原文では」「文字起こしでは」「録音によると」等）。要約は独立した議事録として読まれる
