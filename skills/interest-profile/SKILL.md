...
Claude Code の会話履歴を分析し、ユーザーの興味プロファイルを `~/.claude/INTERESTS.md` に蓄積・更新する。

このスキルの責務は **「会話履歴から興味を抽出して蓄積する」ことだけ** に絞られている。
普段どおり Claude と会話を重ねるほど、その履歴からプロファイルが自動的に育っていく。記事を読んで質問する、調べ物をする、意見を述べる——そうした日常の会話がそのままシグナルになる。
...
データ保存先: `~/.claude/interests/`
プロファイル出力先: `~/.claude/INTERESTS.md`
...
このSKILL.mdと同じディレクトリに同梱された `scripts/extract_interests.py` で会話ログの差分抽出を行う（symlink経由で読み込まれていても、実体のディレクトリを解決してから使う）。

```bash
python3 <このSKILL.mdのディレクトリ>/scripts/extract_interests.py \
  --logs-dir "$HOME/.claude/projects" \
  --recursive \
  --state-file "$HOME/.claude/interests/last-sync.json" \
  --state-out "$HOME/.claude/interests/last-sync.json.pending" \
  --max-messages 500
```

**注意:**
- `--logs-dir "$HOME/.claude/projects" --recursive` により、特定プロジェクトに限らず全プロジェクトの会話履歴が興味プロファイルに反映される。
- 特定プロジェクトのみ対象にしたい場合は `--logs-dir "$HOME/.claude/projects/<プロジェクトのエンコード済みパス>"` で明示指定（`--recursive` 不要）。
...
### Section 2: 既存の蓄積データの読み込み

`~/.claude/interests/interest-log.jsonl` から**直近90日分のシグナルだけ**を読み込む。全行は読まない（ログは無限に増えるため、全部読むとコンテキストが際限なく膨らむ。時間重み上、30日超の古いシグナルはほぼ効かないので直近分で十分）。

スクリプトに直近分だけを出力させ、その出力を読む：

```bash
python3 <このSKILL.mdのディレクトリ>/scripts/extract_interests.py \
  --recent-log "$HOME/.claude/interests/interest-log.jsonl" \
  --recent-days 90
```
...
Section 2で読み込んだ直近90日分のシグナル + 今回の新規シグナルから `~/.claude/INTERESTS.md` を生成する。
...
INTERESTS.md は `~/.claude/INTERESTS.md` に保存する。

### Section 5: 状態の更新

全処理が正常完了した後、以下を**この順序で**実行する。**順序を守ること**（逆順だと部分コミットになり、しおりだけ進んで新規シグナルが未記録になる）：

1. 新規シグナルを `~/.claude/interests/interest-log.jsonl` に追記（1行1JSON）
2. pending ファイル（`last-sync.json.pending`）を本ファイルへ昇格（commit）する：

```bash
python3 <このSKILL.mdのディレクトリ>/scripts/extract_interests.py \
  --state-file "$HOME/.claude/interests/last-sync.json" \
  --commit "$HOME/.claude/interests/last-sync.json.pending"
```
...
`~/.claude/INTERESTS.md` を読み込んで表示する。
