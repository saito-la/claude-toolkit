...
**正本はこのリポジトリで、`~/.claude/` は配置先である。** 配置物を直したくなったら、このリポジトリのファイルを直して commit & push する。symlink 配置の端末では `~/.claude/<名前>` を編集しても同じ実体に当たるが、**そこで止めると次の `git pull` や `git submodule update` で消える。** コピー配置の Windows では `install.py` の再実行で上書きされる。どちらも失敗せず静かに消えるので、消えたことは次に読もうとしたときまで分からない。

実際に 2026-08-29、`guides/SESSION-END.md` へ足した段が push されないまま消え、2週間ぶん気づかれなかった。**`~/.claude/` 配下の配置物を編集したセッションは、終了時に配布元の commit & push まで済ませる**（`~/.claude/.wired-by` に名前があるファイルが配置物）。
